...
# Calculate R-squared
r2_tree_simple <- 1 - sum((test_data$SalePrice - pred_simple)^2) / sum((test_data$SalePrice - mean(test_data$SalePrice))^2)
r2_tree_medium <- 1 - sum((test_data$SalePrice - pred_medium)^2) / sum((test_data$SalePrice - mean(test_data$SalePrice))^2)
r2_tree_complex <- 1 - sum((test_data$SalePrice - pred_complex)^2) / sum((test_data$SalePrice - mean(test_data$SalePrice))^2)
r2_tree_very_complex <- 1 - sum((test_data$SalePrice - pred_very_complex)^2) / sum((test_data$SalePrice - mean(test_data$SalePrice))^2)
...
p1 <- ggplot(tree_comparison, aes(x = Max_Depth, y = RMSE)) +
  geom_line(color = "red", size = 1.2) +
  geom_point(color = "red", size = 3) +
  labs(title = "Decision Trees: Overfitting Problem",
       subtitle = "Performance gets worse with more complexity",
       x = "Max Depth", y = "RMSE") +
  theme_minimal()

# Plot 2: Random Forests (no overfitting)
p2 <- ggplot(performance_df, aes(x = Trees, y = RMSE)) +
  geom_line(color = "blue", size = 1.2) +
  geom_point(color = "blue", size = 3) +
  labs(title = "Random Forests: No Overfitting",
       subtitle = "Performance only gets better with more trees",
       x = "Number of Trees", y = "RMSE") +
  theme_minimal() +
  scale_x_log10()
...
# Plot 1: Decision Trees (showing overfitting)
ax1.plot(tree_comparison_df['Max_Depth'], tree_comparison_df['RMSE'], 'ro-', linewidth=2, markersize=8)
ax1.set_xlabel('Max Depth')
ax1.set_ylabel('RMSE')
ax1.set_title('Decision Trees: Overfitting Problem\nPerformance gets worse with more complexity')
ax1.grid(True, alpha=0.3)

# Plot 2: Random Forests (no overfitting)
ax2.plot(performance_df['Trees'], performance_df['RMSE'], 'bo-', linewidth=2, markersize=8)
ax2.set_xlabel('Number of Trees')
ax2.set_ylabel('RMSE')
ax2.set_title('Random Forests: No Overfitting\nPerformance only gets better with more trees')
ax2.set_xscale('log')
ax2.grid(True, alpha=0.3)
