...
**Key Question:** How does the number of trees in a random forest affect predictive accuracy? Can we improve performance by simply adding more trees?  How much accuracy is gained when sacrificing the interpretability of a decision tree for a random forest?  Does linear regression have comparable performance to a random forest or a complicated decision tree for **this dataset**?

**The Issue:** Individual decision trees with limited depth can be considered "weak learners", there is some signal in their guesses, but it is not very strong.  Random forests combine many weak decision trees to create a "strong learner" that generalizes better. But how many trees do we need? Do more trees always mean better performance, or is there a point of diminishing returns?  How do random forests compare to linear regression and a single decision tree for this dataset?

## Data Loading and Random Forest Models

We are once again analyzing the Ames Housing dataset which contains detailed information about residential properties sold in Ames, Iowa from 2006 to 2010.  It contains one categorical variable (zipCode) and many numerical variables.  A real estate dataset is expected to have a non-linear relationship between the features and the target variable (SalePrice).  For example, additional square footage in a wealthy zip code will have a different effect on the price than additional square footage in a less wealthy zip code.  Since non-linearities are anticiapted, we will use a random forest to model the relationship between the features and the target variable as decision trees and random forests are able to capture non-linear relationships.
...
cat("Built 8 random forests with corrected categorical zipCode: 1, 5, 25, 100, 500, 1,000, 2,000, and 5,000 trees\n")
```
...
print("Built 8 random forests with corrected categorical zipCode: 1, 5, 25, 100, 500, 1,000, 2,000, and 5,000 trees")
```
...
p1 <- ggplot(performance_df, aes(x = Trees, y = RMSE_Test)) +
  ...
  labs(title = "RMSE Decreases with More Trees (Corrected Analysis)",
...
p2 <- ggplot(performance_df, aes(x = Trees, y = R_squared)) +
  ...
  labs(title = "R-squared Increases with More Trees (Corrected Analysis)",
...
ax1.set_ylabel('RMSE')
ax1.set_title('RMSE Decreases with More Trees (Corrected Analysis)')
ax1.set_xscale('log')
...
ax2.set_title('R-squared Increases with More Trees (Corrected Analysis)')
ax2.set_xscale('log')
...
- **Categorical Feature Support:** Proper handling of categorical variables like zip codes improves model accuracy
:::
...
p1 <- ggplot(tree_comparison, aes(x = Max_Depth)) +
  ...
  labs(title = "Decision Trees: Overfitting Problem (Corrected Analysis)",
...
p2 <- ggplot(performance_df, aes(x = Trees)) +
  ...
  geom_line(aes(y = RMSE_Train, color = "Training Data"), size = 1.2) +
  geom_point(aes(y = RMSE_Train, color = "Training Data"), size = 3) +
  labs(title = "Random Forests: No Overfitting (Corrected Analysis)",
...
ax1.set_ylabel('RMSE')
ax1.set_title('Decision Trees: Overfitting Problem (Corrected Analysis)\nTraining improves, test gets worse')
...
ax2.set_xlabel('Number of Trees')
ax2.set_ylabel('RMSE')
ax2.set_title('Random Forests: No Overfitting (Corrected Analysis)\nBoth training and test improve with more trees')
ax2.set_xscale('log')
...
## Key Insights: Why Random Forests Can SometimesOutperform Linear Regression and why it didn't happen here
...
2. **Feature Interactions:** Automatically discovers relationships between variables (e.g., zip code × lot area)
